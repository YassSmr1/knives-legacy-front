<template>
  <div id="Tickets">
    ...
    <div class="wrapper">
      <div class="header_text">
        <div class="header_text_title">
          <div class="header_text_title_top">
            MY TICKETS <span class="sub_title">- The KNIVES</span>
          </div>
          <div class="header_text_title_bot">
            All my participation's in the raffles
          </div>
        </div>
        <a
      ...
      <RaffleInfo :raffle="false" v-on:selectFilter="applyFilter"></RaffleInfo>
      <TicketTable
        :raffles="
          displayedRaffles.filter((raffle) => {
            if (filter === 'all')
              return raffle.participants.some(
                (participant) =>
                  participant.toLowerCase() === account.toLowerCase()
              );
            else
              return (
                raffle.status === filter &&
                raffle.participants.some(
                  (participant) =>
                    participant.toLowerCase() === account.toLowerCase()
                )
              );
...
export default class Tickets extends Vue {
  ...
  /***
   * Apply the filter to the displayed tickets
   * @returns {void}
   */
